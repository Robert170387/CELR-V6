...
| 19 | **`refresh_tokens-acumulados`** — 30 filas en la BD de desarrollo | **No es un bug conocido; es una pregunta abierta.** ¿Son residuos de suites que no limpiaron (reincidencia de la norma de cleanup) o sesiones legítimas de uso real? La diferencia decide si corresponde un barrido puntual — cosmético — o buscar la suite que no limpia — que sí sería un defecto. **No borrar a ciegas:** son FK de `usuarios` y borrarlos expulsa sesiones vivas. | observado 2026-09-26 vía `verify_seed` |
