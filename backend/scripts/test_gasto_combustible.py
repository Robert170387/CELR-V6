...
def main():
    ...
    try:
        ...
        # Limpieza 2.B: DELETE físico de los gastos creados en esta corrida (la API hace
        # soft-delete) + restauración de km_actual al valor inicial para no dejar drift
        # + borrado del refresh token del login de esta corrida.
        db2 = SessionLocal()
        try:
            if creados:
                db2.query(Gasto).filter(Gasto.id.in_(creados)).delete(synchronize_session=False)
